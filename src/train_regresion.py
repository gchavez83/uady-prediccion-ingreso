"""Entrena el modelo de REGRESION (target = POSICION).

Modernizacion del notebook uady_regression.ipynb de Colab:
- lee el dataset local 2026 (no la URL de GitHub),
- nombres de columna reconciliados via common.py,
- sin magics de shell,
- agrega validacion temporal (entrenar <=2025, validar 2026) y guarda un
  reporte de metricas en JSON.

Uso:  .venv\\Scripts\\python.exe src\\train_regresion.py
"""
...
def main() -> None:
    ...
    print(df[C.COL_POSICION].describe())

    ignore = [C.COL_FOLIO, C.COL_ESTATUS, C.COL_CICLO]
    report: dict = {"target": C.COL_POSICION, "n_rows": int(len(df))}
    ...
    setup(
        data=datos, target=C.COL_POSICION, session_id=123,
        ignore_features=ignore, categorical_features=[C.COL_ABV],
        max_encoding_ohe=49, verbose=False,
    )
    ...
    predict_model(modelo, data=no_vistos)
    ...
    if 2026 in df[C.COL_CICLO].unique():
        ...
        setup(
            data=train_t, target=C.COL_POSICION, session_id=123,
            ignore_features=ignore, categorical_features=[C.COL_ABV],
            max_encoding_ohe=49, verbose=False,
        )
    ...
    setup(
        data=df, target=C.COL_POSICION, session_id=123,
        ignore_features=ignore, categorical_features=[C.COL_ABV],
        max_encoding_ohe=49, verbose=False,
    )
    modelo_full = create_model("lightgbm")
    final = finalize_model(modelo_full)
    save_model(final, str(C.MODEL_REGRESION))
    print(f"Guardado: {C.MODEL_REGRESION}.pkl")

    (C.MODELS_DIR / "metrics_regresion.json").write_text(
        json.dumps(report, indent=2, ensure_ascii=False, default=str), encoding="utf-8"
    )
    print("Reporte de metricas: models/metrics_regresion.json")
